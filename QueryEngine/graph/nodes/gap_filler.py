...
async def gap_filler_node(state: QueryAgentState) -> dict:
    ...
    for gq in raw_queries:
        ...
        # support/oppose 倾向于 insight_db（Phase 2 暂时降级为 any，Phase 2.5 接入后切换）
        if item["target_stance"] in ("support", "oppose") and item["target_source"] == "insight_db":
            item["target_source"] = "any"
